    ...
    <div style={{ userSelect: 'none' }} onKeyDown={handleKeyDown} tabIndex={0}>
      ...
      <div style={{ overflow: 'auto', maxHeight: 'calc(100vh - 100px)' }} onMouseDown={e => {
        ...
        <table style={{ borderCollapse: 'collapse', fontSize: 12 }}>
          <thead><tr><th style={{ width: 40, background: '#f1f1f1' }}></th>
            {Array(cols).fill(0).map((_, i) => <th key={i} style={{ width: colWidths[getCol(i)] || 80, background: '#f1f1f1', border: '1px solid #ccc', position: 'relative' }}>{getCol(i)}
          ...
          <tbody>
            {Array(rows).fill(0).map((_, r) => {
              const row = r + 1;
              return <tr key={row}><td data-row="true" style={{ background: '#f1f1f1', border: '1px solid #ccc', textAlign: 'center', height: rowHeights[row] || 20 }}>{row}</td>
                {Array(cols).fill(0).map((_, c) => {
                  ...
                  return <td key={cellId} data-cell={cellId} onDoubleClick={() => { setEditCell(cellId); setEditVal(cells[cellId]?.value || ''); }} onContextMenu={e => { e.preventDefault(); setCtx({ x: e.clientX, y: e.clientY, row, col: getCol(c) }); }}
      ...
      {ctx && <div style={{ position: 'fixed', top: ctx.y, left: ctx.x, background: 'white', border: '1px solid #ccc', padding: 5, zIndex: 1000 }}>
